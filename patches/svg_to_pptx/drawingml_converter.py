...
def convert_svg_to_slide_shapes(
    ...
    # --- Apply CSS <style> rules to element attributes ---
    # xml.etree does not parse CSS, so font-family from <style> never reaches
    # the converter. Extract simple tag-level CSS rules and inject them as
    # element attributes so _get_attr() can find them.
    for defs_elem in root.iter(f'{{{SVG_NS}}}defs'):
        for style_elem in defs_elem:
            if style_elem.tag.replace(f'{{{SVG_NS}}}', '') != 'style':
                continue
            css_text = (style_elem.text or '').strip()
            # Parse simple rules like "text { font-family: SimHei, Arial, sans-serif; }"
    ...
    # Pre-scan root children to identify "card" patterns: a background rect
    # + optional accent bar + text elements. These get merged into a single
    # shape with word_wrap + spAutoFit so text auto-wraps inside the box.
    from .drawingml_elements import _is_card_bg_rect, _is_accent_bar, convert_card_group
    ...
    for i, child in enumerate(root_children):
        if i in absorbed_indices:
            continue
        if not _is_card_bg_rect(child):
            continue

        bg_rect = child
        bg_x = _f(bg_rect.get('x'))
        bg_y = _f(bg_rect.get('y'))
        bg_w = _f(bg_rect.get('width'))
        ...
        for j, sib in enumerate(root_children):
            if j == i or j in absorbed_indices:
                continue
            if _is_accent_bar(sib, bg_rect):
                accent_bar = sib
                accent_indices.append(j)
                break

        card_text_indices: list[int] = []
        card_text_elems: list = []
        # First pass: find candidate texts within the rect bounds
        for j, sib in enumerate(root_children):
            if j == i or j in absorbed_indices or j in accent_indices:
                continue
            tag_j = sib.tag.replace(f'{{{SVG_NS}}}', '')
            if tag_j != 'text':
                continue
            tx = _f(sib.get('x'))
            ty = _f(sib.get('y'))
            # Must be within rect bounds (with small tolerance)
            if tx < bg_x + 5 or tx > bg_right - 5:
                continue
            if ty < bg_y - 2 or ty > bg_bottom + 8:
                continue
            card_text_indices.append(j)
            card_text_elems.append(sib)

        # Tightening heuristic: all card texts must share the same left
        # alignment x (within 6px tolerance). Axis labels and chart labels
        # that happen to fall inside the rect bounds will have different x
        # coordinates and get filtered out.
        if card_text_elems:
            first_x = _f(card_text_elems[0].get('x'))
            aligned = []
            aligned_indices = []
            for k, te in enumerate(card_text_elems):
                tx = _f(te.get('x'))
                if abs(tx - first_x) <= 6:
                    aligned.append(te)
                    aligned_indices.append(card_text_indices[k])
            if len(aligned) >= 2:
        ...
        if card_text_elems:
            group = {
                ...
                'text_elems': card_text_elems,
                'child_indices': [i] + accent_indices + card_text_indices,
            }
            card_groups.append(group)
            absorbed_indices.update(group['child_indices'])
    ...
    # Emit merged card shapes first (in document order)
    card_child_indices_set = absorbed_indices
    for group in card_groups:
        result = convert_card_group(
            group['bg_rect'], group['accent_bar'], group['text_elems'], ctx,
        )
        if result:
            ...
            if m:
                fallback_targets.append((int(m.group(1)), 'card'))

    # Then convert remaining children normally (skip those absorbed into cards)
    for i, child in enumerate(root_children):
        if i in card_child_indices_set:
            continue
