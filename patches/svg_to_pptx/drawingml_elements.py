...
# ---------------------------------------------------------------------------
# Card group: merged rect + text (background box with auto-wrapping text)
# ---------------------------------------------------------------------------

_CARD_BG_COLORS = frozenset({
    'f5f5f5', 'fff5f5', 'fffaf0', 'f0f4f8', 'f8f9fa',
    'edf2f7', 'fef2f2', 'fefce8', 'f0fdf4',
})


def _is_card_bg_rect(elem: ET.Element) -> bool:
    """Check if a <rect> looks like a card background container."""
    ...
    if w < 100 or h < 40:
        return False
    fill_raw = (elem.get('fill') or '').lstrip('#')
    if not fill_raw:
        style = elem.get('style', '')
        m = re.search(r'fill\s*:\s*#([0-9a-fA-F]{3,8})', style)
        fill_raw = m.group(1) if m else ''
    return fill_raw.lower() in _CARD_BG_COLORS


def _is_accent_bar(elem: ET.Element, bg_rect: ET.Element) -> bool:
    """Check if a <rect> is a narrow accent bar aligned with bg_rect."""
    tag = elem.tag.replace(f'{{{SVG_NS}}}', '')
    if tag != 'rect':
        return False
    w = _f(elem.get('width'))
    if w > 10:
        return False
    bg_y = _f(bg_rect.get('y'))
    bg_h = _f(bg_rect.get('height'))
    elem_y = _f(elem.get('y'))
    if abs(elem_y - bg_y) > 5:
        return False
    elem_h = _f(elem.get('height'))
    if abs(elem_h - bg_h) > 5:
        return False
    return True


def convert_card_group(
    bg_rect: ET.Element,
    accent_bar: ET.Element | None,
    text_elems: list[ET.Element],
    ctx: ConvertContext,
) -> ShapeResult | None:
    """Convert a card (background rect + texts) into a single merged shape.

    Width is locked to the background rect. Height uses spAutoFit so the
    shape grows to contain all text. Text wraps within the rect via
    wrap="square".
    """
    rect_x = ctx_x(_f(bg_rect.get('x')), ctx)
    rect_y = ctx_y(_f(bg_rect.get('y')), ctx)
    rect_w = ctx_w(_f(bg_rect.get('width')), ctx)
    rect_h = ctx_h(_f(bg_rect.get('height')), ctx)

    fill_raw = (bg_rect.get('fill') or '').lstrip('#') or 'F5F5F5'
    fill_color = parse_hex_color('#' + fill_raw) or 'F5F5F5'
    fill_op = get_fill_opacity(bg_rect, ctx)
    alpha_xml = ''
    if fill_op is not None and fill_op < 1.0:
        alpha_xml = f'<a:alphaMod val="{int(fill_op * 100000)}"/>'
    fill_xml = f'<a:solidFill><a:srgbClr val="{fill_color}">{alpha_xml}</a:srgbClr></a:solidFill>'

    stroke_op = get_stroke_opacity(bg_rect, ctx)
    stroke_xml = build_stroke_xml(bg_rect, ctx, stroke_op)

    rx_attr = bg_rect.get('rx')
    rx = _f(rx_attr) * ctx.scale_x if rx_attr else 0.0
    if rx > 0:
    ...
    # Left inset: distance from rect left edge to text start
    left_inset_emu = px_to_emu(10)
    if accent_bar is not None:
        bg_x_val = _f(bg_rect.get('x'))
        text_start = min(
            (_f(t.get('x')) for t in text_elems if t.get('x')),
            default=bg_x_val + 14,
        )
        left_inset_emu = max(px_to_emu(text_start - bg_x_val), px_to_emu(6))

    # Build <a:p> paragraphs from each text element
    paragraphs_parts: list[str] = []
    for text_elem in text_elems:
        font_size = _f(_get_attr(text_elem, 'font-size', ctx), 12) * ctx.scale_y
        font_weight = _get_attr(text_elem, 'font-weight', ctx) or '400'
        font_family_str = _get_attr(text_elem, 'font-family', ctx) or ''
        fill_raw_t = _get_attr(text_elem, 'fill', ctx) or '#1A1A1A'
        fill_color_t = parse_hex_color(fill_raw_t) or '1A1A1A'
        ...
        line_spc_val = round(font_size * FONT_PX_TO_HUNDREDTHS_PT * 1.3)
        paragraphs_parts.append(
            '<a:p>\n'
            f'<a:pPr algn="l"><a:lnSpc><a:spcPts val="{line_spc_val}"/></a:lnSpc></a:pPr>\n'
            f'{runs_xml}\n'
            '</a:p>'
        )

    if not paragraphs_parts:
        return None

    paragraphs_xml = '\n'.join(paragraphs_parts)

    shape_id = ctx.next_id()
    off_x = px_to_emu(rect_x)
    off_y = px_to_emu(rect_y)
    ext_cx = px_to_emu(rect_w)
    ext_cy = px_to_emu(rect_h)

    right_inset_emu = px_to_emu(6)
    bottom_inset_emu = px_to_emu(4)

    body_pr_xml = (
        f'<a:bodyPr wrap="square" lIns="{left_inset_emu}" tIns="0" '
        f'rIns="{right_inset_emu}" bIns="{bottom_inset_emu}" '
    ...
    xml_parts = [
        '<p:sp>',
        '<p:nvSpPr>',
        f'<p:cNvPr id="{shape_id}" name="Card {shape_id}"/>',
        '<p:cNvSpPr/><p:nvPr/>',
        '</p:nvSpPr>',
        '<p:spPr>',
